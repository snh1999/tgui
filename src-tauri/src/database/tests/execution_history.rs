...
fn test_get_command_execution_history_default_limit() {
    ...
    for _ in 0..5 {
        ...
        test_db
            .db
            .update_execution_history_status(execution_id, ExecutionStatus::Failed, None)
            .unwrap();
    }
...
fn test_get_command_execution_history_with_limit() {
    ...
    for _ in 0..5 {
        ...
        test_db
            .db
            .update_execution_history_status(execution_id, ExecutionStatus::Cancelled, None)
            .unwrap();
    }
...
fn test_update_execution_pid_rejects_non_running_status() {
    ...
    test_db
        .db
        .update_execution_history_status(id, ExecutionStatus::Failed, Some(1))
        .unwrap();
...
fn test_update_execution_history_status_rejects_running_to_running() {
    ...
    let result = test_db
        .db
        .update_execution_history_status(id, ExecutionStatus::Running, None);
    assert!(matches!(
        result,
        Err(DatabaseError::InvalidData {
            field: "status",
...
fn test_cleanup_history_older_than_zero_days_deletes_all_non_running() {
    ...
    // Non-running, old
    let finished = test_db
        .save_execution_history(&ExecutionHistoryBuilder::new().with_command(cmd_id).build());
    test_db
        .db
        .update_execution_history_status(finished, ExecutionStatus::Success, Some(0))
        .unwrap();
    test_db
        .db
        .conn()
...
fn test_cleanup_command_history_preserves_workflow_associated_entries() {
    ...
    test_db
        .db
        .update_execution_history_status(workflow_h, ExecutionStatus::Success, Some(0))
        .unwrap();
...
fn test_get_execution_stats_command_success_rate() {
    ...
    let success1 = test_db
        .save_execution_history(&ExecutionHistoryBuilder::new().with_command(cmd_id).build());
    test_db
        .db
        .update_execution_history_status(success1, ExecutionStatus::Success, Some(0))
        .unwrap();

    let success2 = test_db
        .save_execution_history(&ExecutionHistoryBuilder::new().with_command(cmd_id).build());
    test_db
        .db
        .update_execution_history_status(success2, ExecutionStatus::Success, Some(0))
        .unwrap();
    ...
    test_db
        .db
        .update_execution_history_status(failed, ExecutionStatus::Failed, Some(1))
        .unwrap();

    let cancelled = test_db
        .save_execution_history(&ExecutionHistoryBuilder::new().with_command(cmd_id).build());
    test_db
        .db
        .update_execution_history_status(cancelled, ExecutionStatus::Cancelled, None)
        .unwrap();
...
fn test_get_execution_stats_workflow_excludes_steps() {
    ...
    test_db
        .db
        .update_execution_history_status(flow_exec, ExecutionStatus::Success, Some(0))
        .unwrap();
    ...
    test_db
        .db
        .update_execution_history_status(step_exec, ExecutionStatus::Failed, Some(1))
        .unwrap();

    let stats = test_db
        .db
        .get_execution_stats(StatsTarget::Workflow(flow_id), None)
        .unwrap();
...
fn test_get_execution_stats_global_excludes_steps() {
    ...
    // Standalone command
    let cmd_exec = test_db
        .save_execution_history(&ExecutionHistoryBuilder::new().with_command(cmd_id).build());
    test_db
        .db
        .update_execution_history_status(cmd_exec, ExecutionStatus::Success, Some(0))
        .unwrap();
    ...
    test_db
        .db
        .update_execution_history_status(flow_exec, ExecutionStatus::Failed, Some(1))
        .unwrap();
    ...
    let stats = test_db
        .db
        .get_execution_stats(StatsTarget::Global, None)
        .unwrap();
...
fn test_get_execution_stats_duration_calculation() {
    ...
    test_db
        .db
        .update_execution_history_status(exec2, ExecutionStatus::Failed, Some(1))
        .unwrap();
...
fn test_update_execution_pid_large_value() {
    ...
    test_db
        .db
        .update_execution_pid(history_id, large_pid)
        .unwrap();
...
fn test_get_execution_stats_counts_all_status_types() {
    ...
    for (status, exit_code) in &statuses {
        ...
        if *status != ExecutionStatus::Running {
            test_db
                .db
                .update_execution_history_status(exec_id, status.clone(), *exit_code)
                .unwrap();
        }
    }
...
fn test_get_execution_stats_timestamps_ordered() {
    ...
    test_db
        .db
        .update_execution_history_status(exec2, ExecutionStatus::Success, Some(0))
        .unwrap();
...
fn test_get_execution_stats_global_with_time_filter() {
    ...
    test_db
        .db
        .update_execution_history_status(old, ExecutionStatus::Success, Some(0))
        .unwrap();
    test_db
        .db
        .conn()
    ...
    test_db
        .db
        .update_execution_history_status(recent, ExecutionStatus::Failed, Some(1))
        .unwrap();
    ...
    let last_7 = test_db
        .db
        .get_execution_stats(StatsTarget::Global, Some(7))
        .unwrap();
...
fn test_get_execution_stats_with_time_filter() {
    ...
    let old_exec = test_db
        .save_execution_history(&ExecutionHistoryBuilder::new().with_command(cmd_id).build());
    test_db
        .db
        .update_execution_history_status(old_exec, ExecutionStatus::Success, Some(0))
        .unwrap();
    ...
    let recent_exec = test_db
        .save_execution_history(&ExecutionHistoryBuilder::new().with_command(cmd_id).build());
    test_db
        .db
        .update_execution_history_status(recent_exec, ExecutionStatus::Failed, Some(1))
        .unwrap();

    // All time stats - should see both
    let all_time = test_db
        .db
        .get_execution_stats(StatsTarget::Command(cmd_id), None)
        .unwrap();
    ...
    let last_week = test_db
        .db
        .get_execution_stats(StatsTarget::Command(cmd_id), Some(7))
        .unwrap();
    ...
    let last_day = test_db
        .db
        .get_execution_stats(StatsTarget::Command(cmd_id), Some(1))
        .unwrap();
...
fn test_get_execution_stats_time_filter_averages_and_timestamps() {
    ...
    assert_eq!(
        recent_stats.first_executed_at,
        recent_stats.last_executed_at
    ); // Only one record
}
...
fn test_get_latest_execution_for_command_returns_most_recent() {
    ...
    test_db
        .db
        .update_execution_history_status(first, ExecutionStatus::Success, Some(0))
        .unwrap();
    std::thread::sleep(std::time::Duration::from_millis(10));
