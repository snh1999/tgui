...
fn test_get_running_commands_all() {
    ...
    test_db
        .db
        .update_execution_history_status(history_id2, ExecutionStatus::Success, Some(0))
        .unwrap();
    test_db
        .db
        .update_execution_history_status(history_id3, ExecutionStatus::Failed, Some(100))
        .unwrap();
...
fn test_get_running_commands_by_command_id() {
    ...
    let running = test_db
        .db
        .get_running_commands(Some(cmd_id1), None)
        .unwrap();
...
fn test_get_running_commands_by_workflow_id() {
    ...
    let running = test_db
        .db
        .get_running_commands(None, Some(flow_id))
        .unwrap();
    assert_eq!(running.len(), 2);
}
...
fn test_get_command_execution_stats() {
    ...
    let cancelled = test_db
        .db
        .get_command_execution_stats(cmd_id, Some(ExecutionStatus::Cancelled))
        .unwrap();
...
fn test_execution_history_stats_after_mixed_runs() {
    ...
    assert_eq!(
        test_db
            .db
            .get_command_execution_stats(cmd_id, Some(ExecutionStatus::TimeOut))
            .unwrap(),
        1
    );
}
