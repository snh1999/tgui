...
export function sanitizeHexInput(raw: string): string {
    const body = raw.replace(/^#/, '').replace(/[^0-9a-fA-F]/g, '').slice(0, 8)
    if (body.length <= 4) {
...
export function useColorPicker() {
    ...
    const currentRgb = computed(() => hsvToRgb(hue.value, saturation.value, brightness.value))
    const currentHex = computed(() => rgbToHex(currentRgb.value.r, currentRgb.value.g, currentRgb.value.b))
    const currentHsl = computed(() => hsvToHsl(hue.value, saturation.value, brightness.value))
    ...
    const currentRgba = computed(() => `rgba(${currentRgb.value.r},${currentRgb.value.g},${currentRgb.value.b},${alpha.value})`)

    const currentHexWithAlpha = computed(() =>
        alpha.value < 1 ? currentHex.value + alphaToHex(alpha.value) : currentHex.value
    )
    ...
    return {
        ...
        currentHex, currentHexWithAlpha, hueColor, currentRgba, displayValue,
        hexInput, rgbInputs, hslInputs,
        applyFromHex, applyHex, applyRgb, applyHsl, applyPreset,
    }
}
