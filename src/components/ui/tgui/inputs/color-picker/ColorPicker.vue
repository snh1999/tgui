...
<template>
  <div class="select-none border border-input p-4 space-y-3.5">
    <div>
      <p
        class="text-[10px] font-medium uppercase tracking-widest text-muted-foreground mb-2"
      >
        Presets
      </p>
    ...
    <Tabs v-model="format">
      ...
      <TabsContent value="hsl">
        <div class="grid grid-cols-4 gap-1.5">
          <div
            v-for="(max, ch) in ({ h: 360, s: 100, l: 100, a: 100 } as const)"
            :key="ch"
            class="flex flex-col gap-1"
          >
            <InputGroup>
