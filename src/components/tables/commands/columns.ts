        ...
        cell: ({row}) => {
            ...
            return h('div', {class: 'flex items-center gap-3 min-w-0'}, [
                ...
                h('div', {class: 'flex flex-col min-w-0'}, [
                    ...
                    command.description
                        ? h('span', {class: 'text-xs text-muted-foreground truncate'}, command.description)
                        : null
                ])
            ])
        }
    ...
    },
    {
        ...
        cell: ({row}) => {
            ...
            return h('div', {class: 'flex flex-col gap-0.5'}, [
                h('span', {class: 'text-sm text-muted-foreground'}, command.activityTime),
                command.pid
                    ? h('span', {class: 'text-xs font-mono text-muted-foreground/70'}, `PID: ${command.pid}`)
                    : null
            ])
        }
    },
