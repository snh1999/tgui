...
**Threat Model**: TGUI runs locally on user machine with full user privileges. Primary risks are accidental data loss, command injection, unintended execution and insecure storage of sensitive data. So security is critical even for personal use.
...
- **Mitigation**:
  - **UI Warning**: Display prominent warning when adding env vars:
    > "Security Notice: Environment variables are stored in plain text. Avoid storing passwords, API keys, or other sensitive data unless you accept this risk."
...
// In config.json
{
  "allowed_commands": ["npm", "python", "tail", "grep"], // If empty, allow all
  "blocked_commands": ["rm", "dd", "mkfs"] // Explicitly forbid dangerous commands
}
...
// In tauri.conf.json
{
  "plugins": {
    "updater": {
      ...
      "endpoints": ["https://yourdomain.com/releases.json"]
    }
  }
}
```
...
### HTTPS Only
- Always use `https://` for updater endpoints
- Tauri will reject `http://` by default
...
```bash
tauri sign generate
# Keep private key in GitHub Secrets: TAURI_PRIVATE_KEY
